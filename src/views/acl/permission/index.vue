<template>
  <div class="permission-box">
    <el-table :data="permissionArr" border v-loading="listLoading" row-key="id" style="width: 100%">
      <el-table-column label="名称" prop="name"></el-table-column>
      <el-table-column label="权限值" prop="code"></el-table-column>
      <el-table-column label="修改时间" prop="updateTime"></el-table-column>
      <el-table-column label="操作">
        <template #="{ row }">
          <el-button type="primary" size="small" icon="User" v-show="row.level != 4" @click="addPermission(row)">
          ...
          <el-popconfirm :title="'您确定要删除该' + (row.level == 4 ? '功能' : '菜单') + '吗?'" confirm-button-text="确定"
            cancel-button-text="取消" @confirm="removePermission(row)" :width="220">
            <template #reference>
    ...
    <el-dialog v-model="dialogVisible" :title="menuData.id ? '更新' : '添加'" width="30%">
...
import { reqAllPermission, reqAddOrUpdatePermission, reqRemovePeremission } from '@/api/acl/permission/index';
import type { PermissionResponseData, PermissionList, PermissionParams, Permission } from '@/api/acl/permission/type'
