  ...
  <div>
    ...
    <el-card style="margin-top: 20px;">
      <div v-show="scene == 0">
        ...
        <el-table :data="attrArr" v-loading="loading" border style="margin:10px 0;">
          <el-table-column label="序号" type="index" width="100" align="center"></el-table-column>
          <el-table-column label="三级分类id" width="100" prop="categoryId" align="center"></el-table-column>
          <el-table-column label="属性id" width="100" prop="id" align="center"></el-table-column>
          <el-table-column label="属性名称" width="200" prop="attrName"></el-table-column>
          <el-table-column label="属性值名称">
            <template #="{ row }">
              <div>
                <el-tag class="tag_item" type="success" v-for="item in row.attrValueList">{{ item.valueName
                }}</el-tag>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="200">
            <template #="{ row }">
              ...
              <el-button type="primary" size="small" icon="Delete" @click="deleteAttr(row)"></el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
      <div v-show="scene == 1">
        ...
        <el-table border style="margin:10px 0;" :data="attrParams.attrValueList">
          ...
          <el-table-column label="属性值名称">
            <template #="{ row, $index }">
              <el-input v-if="flag" @blur="toLook(row, $index)" placeholder="请输入属性值名称" v-model="row.valueName"></el-input>
              <div @click="toEdit(row)" v-else>{{ row.valueName }}</div>
            </template>
          </el-table-column>
          <el-table-column label="属性值操作" width="200"></el-table-column>
        </el-table>
        <el-button type="primary" size="default" :disabled="attrParams.attrValueList.length < 1"
...
import type { AttrResponseData, AttrDataObj, attrValueType } from '@/api/product/attr/type';
...
watch(() => categoryStore.c3Id, (newValue) => {
  attrArr.value = [];
