    ...
    <el-table :data="skuArr" :height="skuTableHeight" v-loading="skuLoading" width="500px" border
      ...
      <el-table-column label="操作" align="center" width="205" fixed="right">
        <template #="{ row }">
          ...
          <el-popconfirm title="您确认要删除该条数据吗?" confirm-button-text="确定" cancel-button-text="取消" @confirm="deleteSku(row)"
            :width="220">
            <template #reference>
        ...
        <el-row class="detail_row">
          ...
          <el-col :span="18">
            <el-tag v-for="item in skuInfo.skuSaleAttrValueList" :key="item.id"
              style="margin-right:5px;margin-bottom:5px;">{{ item.saleAttrValueName }}</el-tag>
          </el-col>
        </el-row>
        <el-row class="detail_row">
...
import { reqSkuList, reqSaleSku, reqCanleSale, reqSkuInfo, reqDeleteSku } from '@/api/product/sku/index';
import type { SkuResponseData, SkuData, SkuInfoResponse, RestSkuInfo } from '@/api/product/sku/type';
